...
- 所有哈希表的默认起始大小4
- 字典里包含2个hash表，rehash的条件是 used/size > 1，且此时rehashidx > -1，此时2个hash表里都有内容，查询可能在2个hash里搜索；rehash结束后只有ht[0]里有内容，ht[1]为空，rehashidx=-1
- rehash操作不是一次性完成，而是渐进式的，增删查改操作都会尝试执行一段时间的rehash，一些slot里的链表的节点会移动到ht[1]里
- 字典里插入一个元素，先查找，如果有就返回，否则就新加；新加元素放在slot里，采用头插法
- 跟定key查找在字典里，先用hashFunction运算得到h，然后与ht[0].size执行逻辑与操作，如果在执行rehash，还要与ht[1].sizemask执行逻辑与
- 删除一个元素，先判断ht[0]里是否有内容，如果没有直接退出；否则根据key查找在字典里位置，找到后删掉该元素
- 除了常规使用的hash类型，Redis所有的key都放在16个db里，每个db是一个字典，根据key查找内容时候都要涉及到在字典里查找内容操作：dictFind()
